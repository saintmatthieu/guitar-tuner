...
TEST(PitchDetectorImpl, benchmarking) {
  ...
  for (const auto &sample : samples) {
    ...
    for (const auto &noise : noiseData) {
      ...
      const auto resultPath =
          testUtils::getOutDir() / (cleanFile.string() + "_results.py");
      const auto rmsError =
          testUtils::writeResultFile(sample, results, resultPath);
      rmsErrors.push_back(rmsError);

      std::cout << "RMS error for " << testUtils::getFileShortName(testFile)
                << " with noise " << testUtils::getFileShortName(noise.file)
                << " at " << noise.rmsDb << " dB: " << rmsErrors.back()
                << " cents\n";
  ...
  const auto classifierQualityIsUnchanged = testUtils::valueIsUnchanged(
